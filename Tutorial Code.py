...
def main(i):
    ...
    while keep_alive == True:
        ...
        for count, channel in enumerate(eeg_channels):
            ...
            if count == 0:
                DataFilter.perform_bandstop(data[channel], sampling_rate, 4.0, 60.0, 4, FilterTypes.BUTTERWORTH.value, 0)  # bandstop 58 - 62
                DataFilter.perform_bandpass(data[channel], sampling_rate, 20.0, 21.0, 4, FilterTypes.BESSEL.value, 0)  # bandpass 11 - 31
            if count == 1:
                DataFilter.perform_bandstop(data[channel], sampling_rate, 4.0, 60.0, 4, FilterTypes.BUTTERWORTH.value, 0)  # bandstop 58 - 62
                DataFilter.perform_bandpass(data[channel], sampling_rate, 20.0, 21.0, 4, FilterTypes.BESSEL.value, 0)  # bandpass 11 - 31
            if count == 2:
